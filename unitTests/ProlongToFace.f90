...
REAL                           :: Uvol_nVar(PP_nVarDet,0:NRef,0:NRef,0:NRefZ,nElemsRef)
REAL                           :: Uface_master(PP_nVarDet,0:NRef,0:NRefZ,1:nSidesRef),Uface_master_ref(0:NRef,0:NRefZ,1:nSidesRef)
REAL                           :: Uface_slave(PP_nVarDet,0:NRef,0:NRefZ,1:nSidesRef)!,Uface_slave_ref(0:9,0:9,7:6)
#if FV_ENABLED
REAL                           :: FV_Uface_master(PP_nVarDet,0:NRef,0:NRefZ,1:nSidesRef),FV_Uface_master_ref(0:NRef,0:NRefZ,1:nSidesRef)
REAL                           :: FV_Uface_slave(PP_nVarDet,0:NRef,0:NRefZ,1:nSidesRef)
#endif
...
FV_Elems_slave = 0
#endif
CALL ProlongToFace(PP_nVarDet,NRef,Uvol_nVar,Uface_master,Uface_slave,L_Minus,L_Plus,.FALSE.)
#if FV_ENABLED
FV_Elems = 1
FV_Elems_master = 1
FV_Elems_slave = 1
CALL ProlongToFaceCons(PP_nVarDet,NRef,Uvol_nVar,FV_Uface_master,FV_Uface_slave,L_Minus,L_Plus,.FALSE.)
#endif
...
ELSE
  ...
  IF (binaryExists) THEN
    ...
    equal =  .TRUE.
    DO i=1,PP_nVarDet; DO j=0,NRef; DO k=0,NRefZ; DO l=1,nSidesRef
