...
! LOCAL VARIABLES
INTEGER            :: iElem
INTEGER            :: N_Base,nVar_Base,nElems_Base,nSGElems_Base
...
CALL GetDataProps(nVar_Base,N_Base,nElems_Base,nSGElems_Base,NodeType_Base)
...
IF((N_Base.EQ.PP_N).AND.(TRIM(NodeType_Base).EQ.TRIM(NodeType)))THEN
  ...
  CALL ReadArray('DG_Solution',6,(/PP_nVar,PP_N+1,PP_N+1,PP_NZ+1,nElems,1/),OffsetElem,5,RealArray=SpBaseFlow,&
                 offset_stoch=mySGElem-1)
ELSE
  ...
  CALL ReadArray('DG_Solution',6,(/PP_nVar,N_Base+1,N_Base+1,N_Base+1,nElems,1/),OffsetElem,5,RealArray=UTmp,&
                 offset_stoch=mySGElem-1)
  DO iElem=1,nElems
