...
USE MOD_Equation_Vars     ,ONLY: StrVarNames,StrVarNamesDet
#if PP_dim == 2
...
IF(MPIRoot) CALL GenerateFileSkeleton(TRIM(FileName),'State',PP_nVar,NOut,StrVarNames,MeshFileName,OutputTime,&
                                      FutureTime,withUserblock=.TRUE.,StrVarNamesDet=StrVarNamesDet)
...
CALL GatheredWriteArray(FileName,create=.FALSE.,&
                        ...
                        nValGlobal=(/PP_nVar,NOut+1,NOut+1,NOut+1,nGlobalElems,nSGElems/),&
                        nVal=nVal                                              ,&
                        offset=    (/0,      0,     0,     0,     offsetElem,  mySGElem-1/),&
...
IF(gatheredWrite)THEN
  ...
  IF(MPILocalRoot)THEN
    ...
    CALL OpenDataFile(FileName,create=create,single=.FALSE.,readOnly=.FALSE.,communicator=MPI_COMM_LEADERS)
...
ELSE
#endif
  CALL OpenDataFile(FileName,create=create,single=.FALSE.,readOnly=.FALSE.,communicator=MPI_COMM_WORLD)
...
ALLOCATE(VarNames(nVarLoc))
ALLOCATE(ElemData(nVarLoc,nElems))
...
DO WHILE(ASSOCIATED(e))
  nVarLoc=nVarLoc+1
  VarNames(nVarLoc)=e%VarName
  IF(ASSOCIATED(e%RealArray))  ElemData(nVarLoc,:)=e%RealArray
  IF(ASSOCIATED(e%RealScalar)) ElemData(nVarLoc,:)=e%RealScalar
  IF(ASSOCIATED(e%IntArray))   ElemData(nVarLoc,:)=REAL(e%IntArray)
  IF(ASSOCIATED(e%IntScalar))  ElemData(nVarLoc,:)=REAL(e%IntScalar)
  IF(ASSOCIATED(e%eval))       CALL e%eval(ElemData(nVarLoc,:)) ! function fills elemdata
  e=>e%next
END DO

!IF(MPIRoot) CALL GenerateFileSkeleton(TRIM(FileName),'State',nVarLoc,0,VarNames,'',0.,DataSet='ElemData',create=.FALSE.)
!#if USE_MPI
!CALL MPI_BARRIER(MPI_COMM_WORLD,iError)
!#endif
IF(MPIRoot)THEN
  CALL OpenDataFile(FileName,create=.FALSE.,single=.TRUE.,readOnly=.FALSE.,communicator=-1)
  CALL WriteAttribute(File_ID,'VarNamesAdd',nVarLoc,StrArray=VarNames)
  CALL CloseDataFile()
END IF
CALL GatheredWriteArray(FileName,create=.FALSE.,&
                        DataSetName='ElemData', rank=3,  &
                        nValGlobal=(/nVarLoc,nGlobalElems,nSGElems/),&
                        nVal=      (/nVarLoc,nElems      ,1/),&
                        offset=    (/0      ,offSetElem  ,mySGElem-1/),&
...
DO WHILE(ASSOCIATED(f))
  IF(.NOT.f%doSeparateOutput) nVarLoc=nVarLoc+f%nVal(1)
...
IF(MPIRoot.AND.(nVarTotal.NE.nVarLoc))THEN
  CALL OpenDataFile(FileName,create=.FALSE.,single=.TRUE.,readOnly=.FALSE.,communicator=-1)
  f=>FieldList
  DO WHILE(ASSOCIATED(f))
...
DO WHILE(ASSOCIATED(f))
  IF(f%doSeparateOutput)THEN
    ...
    CALL GatheredWriteArray(FileName,create=.FALSE.,&
                            ...
                            nValGlobal=(/f%nVal ,nGlobalElems,nSGElems/),&
...
! --------------------------------------------------------------------------------------------- !
IF(nVarLoc.LE.0) RETURN ! no standard data present

ALLOCATE(VarNames(nVarLoc))
ALLOCATE(tmp(nVarLoc,0:PP_N,0:PP_N,0:PP_NZ,nElems))
...
IF(MPIRoot)THEN
  ...
  DO WHILE(ASSOCIATED(f))
    IF(.NOT.f%doSeparateOutput)THEN
      VarNames(nVarLoc+1:nVarLoc+f%nVal(1))=f%VarNames
      nVarLoc=nVarLoc+f%nVal(1)
    END IF
    f=>f%next
  END DO
  CALL OpenDataFile(FileName,create=.FALSE.,single=.TRUE.,readOnly=.FALSE.,communicator=-1)
  CALL WriteAttribute(File_ID,'VarNamesAddField',nVarLoc,StrArray=VarNames)
  CALL CloseDataFile()
END IF
...
DO WHILE(ASSOCIATED(f))
  IF(.NOT.f%doSeparateOutput)THEN
    IF(ASSOCIATED(f%RealArray))THEN ! real array
      tmp(nVarLoc+1:nVarLoc+f%nVal(1),:,:,:,:)=f%RealArray
    ELSEIF(ASSOCIATED(f%Eval))THEN  ! eval function
      CALL f%Eval(tmp(nVarLoc+1:nVarLoc+f%nVal(1),:,:,:,:))
    END IF
    nVarLoc=nVarLoc+f%nVal(1)
  ENDIF
  f=>f%next
...
CALL GatheredWriteArray(FileName,create=.FALSE.,&
                        ...
                        nValGlobal=(/nVarLoc,PP_N+1,PP_N+1,PP_NZ+1,nGlobalElems,nSGElems/),&
                        nVal=      (/nVarLoc,PP_N+1,PP_N+1,PP_NZ+1,nElems      ,1/),&
                        offset=    (/0      ,0     ,0     ,0      ,offsetElem  ,mySGElem-1/),&
...
! Write DG solution
#if USE_MPI
CALL MPI_BARRIER(MPI_COMM_SGELEM,iError)
#endif
CALL GatheredWriteArray(FileName,create=.FALSE.,&
                        DataSetName='DG_Solution', rank=6,&
                        nValGlobal=(/PP_nVar,PP_N+1,PP_N+1,NZ_loc+1,nGlobalElems,nSGElems/),&
...
IF(MPIRoot)THEN
  ...
  CALL OpenDataFile(TRIM(FileName),create=.FALSE.,single=.TRUE.,readOnly=.FALSE.,communicator=-1)
...
IF(MPISGElemRoot) CALL MarkWriteSuccessfull(FileName)
...
SUBROUTINE GenerateFileSkeleton(FileName,TypeString,nVar,NData,StrVarNames,MeshFileName,OutputTime,&
                                FutureTime,Dataset,create,withUserblock,StrVarNamesDet)
! MODULES
USE MOD_PreProc
USE MOD_Globals
...
#endif
USE MOD_SG_Vars            ,ONLY: nQP,Distribution,StochVarNames
IMPLICIT NONE
!----------------------------------------------------------------------------------------------------------------------------------
! INPUT/OUTPUT VARIABLES
...
CHARACTER(LEN=*)               :: StrVarNames(nVar)  !< Variable names
CHARACTER(LEN=*),OPTIONAL      :: StrVarNamesDet(PP_nVarDet)  !< Variable names
CHARACTER(LEN=*),INTENT(IN)    :: MeshFileName       !< Name of mesh file
REAL,INTENT(IN)                :: OutputTime         !< Time of output
REAL,INTENT(IN),OPTIONAL       :: FutureTime         !< Time of next output
...
CALL OpenDataFile(TRIM(FileName),create=create_loc,single=.TRUE.,readOnly=.FALSE.,&
                  userblockSize=MERGE(userblock_total_len,0,withUserblock_loc),communicator=-1)

! Preallocate the data space for the dataset.
IF(TRIM(Dataset_Str).EQ.'ElemData') THEN
  nDim=3
  Dimsf(1:3) = (/nVar,nGlobalElems,nSGElems/)
ELSE
  nDim=6
  Dimsf=(/nVar,NData+1,NData+1,NData+1   ,nGlobalElems,nSGElems/)
  IF(output2D)                 Dimsf(4)=1
END IF

! Create the dataset with default properties.
HDF5DataType=H5T_NATIVE_DOUBLE
CALL H5SCREATE_SIMPLE_F(nDim, Dimsf(1:nDim), FileSpace, iError)
...
IF(PRESENT(StrVarNamesDet)) CALL WriteAttribute(File_ID,'VarNamesDet',PP_nVarDet,StrArray=StrVarNamesDet)
...
IF(create_loc)THEN
  ...
  CALL WriteAttribute(File_ID,'PP_nDimStoch' ,1           ,IntScalar=PP_nDimStoch)
  CALL WriteAttribute(File_ID,'StochVarNames',PP_nDimStoch,StrArray =StochVarNames)
  CALL WriteAttribute(File_ID,'PP_M'         ,1           ,IntScalar=PP_M)
  CALL WriteAttribute(File_ID,'nQP'          ,1           ,IntScalar=nQP)
  CALL WriteAttribute(File_ID,'Distribution' ,1           ,IntScalar=Distribution)
  CALL WriteAttribute(File_ID,'nSGElems'     ,1           ,IntScalar=nSGElems)
  CALL WriteAttribute(File_ID,'nSGElems_nD'  ,PP_nDimStoch,IntArray =nSGElems_nD)
END IF
