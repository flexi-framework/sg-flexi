...
!==================================================================================================================================
FUNCTION ISVALIDHDF5FILE(FileName,ProgramName,FileType,FileVersion,communicator)
! MODULES
USE MOD_Globals
IMPLICIT NONE
!----------------------------------------------------------------------------------------------------------------------------------
...
INTEGER,INTENT(IN)             :: communicator    !< only MPI and single=F: communicator to be used for collective access
...
CALL H5PSET_FAPL_MPIO_F(Plist_ID,communicator, MPIInfo, iError)
#endif /*USE_MPI*/
...
!==================================================================================================================================
FUNCTION ISVALIDMESHFILE(MeshFileName,communicator)
! MODULES
USE MOD_Globals
IMPLICIT NONE
!----------------------------------------------------------------------------------------------------------------------------------
...
!==================================================================================================================================
SUBROUTINE GetDataProps(nVar_HDF5,N_HDF5,nElems_HDF5,nSGElems_HDF5,NodeType_HDF5)
! MODULES
USE MOD_Globals
IMPLICIT NONE
!----------------------------------------------------------------------------------------------------------------------------------
...
INTEGER,INTENT(OUT)                     :: nSGElems_HDF5  !< number of stoch intervals
...
CHECKSAFEINT(Dims(6),4)
nSGElems_HDF5 = INT(Dims(6),4)
SWRITE(UNIT_stdOut,'(A3,A30,A3,I33,A13)')' | ','Stoch Intervals',' | ',nSGElems_HDF5,' | HDF5    | '
...
!===================================================================================================================================
SUBROUTINE GetArrayAndName(ArrayName,AttribName,nVal,Array,VarNames,offset_stoch)
! MODULES
USE MOD_Globals
...
INTEGER,INTENT(IN),OPTIONAL     :: offset_stoch             !< offset =0, start at beginning of the array
!----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
LOGICAL  :: found
INTEGER  :: dims
...
IF (found) THEN
  ...
  IF(nVal(dims-1).NE.nGlobalElems) STOP 'Last array dimension != nElems !'
  nVal(dims-1)=nElems
  ...
  ! read array
  IF(PRESENT(offset_stoch)) THEN
    nVal(dims)=1
    ALLOCATE(array(PRODUCT(nVal(1:dims))))
    CALL ReadArray(TRIM(ArrayName),dims,nVal(1:dims),OffsetElem,dims,RealArray=array,offset_stoch=offset_stoch)
  ELSE
    ALLOCATE(array(PRODUCT(nVal(1:dims))))
...
!==================================================================================================================================
SUBROUTINE ReadArray(ArrayName,Rank,nVal,Offset_in,Offset_dim,RealArray,IntArray,StrArray,offset_stoch)
! MODULES
USE MOD_Globals
...
IF(PRESENT(offset_stoch)) Offset(Rank)=offset_stoch
