...
ALLOCATE(StrVarNames(PP_nVar))    !< RP variables names for output
...
!----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
CHARACTER(LEN=255)             :: FileString,mySGElemString
REAL                           :: startT,endT
!==================================================================================================================================
...
IF(myRPrank.EQ.0)THEN
  CALL OpenDataFile(Filestring,create=.NOT.RP_fileExists,single=.TRUE.,readOnly=.FALSE.,communicator=-1)
...
CALL OpenDataFile(Filestring,create=.FALSE.,single=.FALSE.,readOnly=.FALSE.,communicator=RP_COMM)
#else
CALL OpenDataFile(Filestring,create=.FALSE.,single=.TRUE. ,readOnly=.FALSE.,communicator=-1)
#endif
...
IF(iSample.GT.0)THEN
  ...
  CALL WriteArray(DataSetName='RP_Data_'//TRIM(mySGElemString), rank=3,&
