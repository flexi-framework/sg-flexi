...
DO SideID=firstSideID,lastSideID
  ...
  IF(ElemID.GT.0)THEN
    IF (FV_Elems_master(SideID).EQ.0) THEN ! DG element
      ...
      END DO; END DO ! p,q
#if   (PP_NodeType==1)
      CALL DoSurfIntPrim(PP_nVarPrim,Nloc,FluxTmp,L_HatMinus,   L_HatPlus,      locSideID,gradU(:,:,:,:,ElemID))
#elif (PP_NodeType==2)
      CALL DoSurfIntPrim(PP_nVarPrim,Nloc,FluxTmp,L_HatMinus(0),L_HatPlus(Nloc),locSideID,gradU(:,:,:,:,ElemID))
#endif
    END IF
  END IF
  ...
  IF(nbElemID.GT.0)THEN
    IF (FV_Elems_slave(SideID).EQ.0) THEN ! DG element
      ...
      END IF
#if   (PP_NodeType==1)
      CALL DoSurfIntPrim(PP_nVarPrim,Nloc,FluxTmp,L_HatMinus,   L_HatPlus,      nblocSideID,gradU(:,:,:,:,nbElemID))
#elif (PP_NodeType==2)
      CALL DoSurfIntPrim(PP_nVarPrim,Nloc,FluxTmp,L_HatMinus(0),L_HatPlus(Nloc),nblocSideID,gradU(:,:,:,:,nbElemID))
#endif
    END IF
  END IF
