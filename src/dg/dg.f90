...
!----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
REAL,DIMENSION(0:N_in,0:N_in)              :: Mass,Minv
INTEGER                                    :: iMass
!==================================================================================================================================
...
Minv=0.
DO iMass=0,N_in
  Mass(iMass,iMass)=wGP(iMass)
  Minv(iMass,iMass)=1./wGP(iMass)
END DO
D_Hat  = -MATMUL(Minv,MATMUL(TRANSPOSE(D),Mass))
...
! 0. Convert Volume solution to primitive
CALL ConsToPrimSG(PP_N,UPrim,U)
...
CALL ProlongToFaceCons(PP_nVar,PP_N,U,U_master,U_slave,L_Minus,L_Plus,doMPISides=.TRUE.)
CALL U_MortarCons(PP_nVar,U_master,U_slave,doMPISides=.TRUE.)
...
CALL U_MortarPrim(PP_nVarPrim,FV_multi_master,FV_multi_slave,doMPiSides=.TRUE.)
...
! 1.1)
CALL ProlongToFaceCons(PP_nVar,PP_N,U,U_master,U_slave,L_Minus,L_Plus,doMPISides=.FALSE.)
CALL U_MortarCons(PP_nVar,U_master,U_slave,doMPISides=.FALSE.)
#if FV_ENABLED
! 1.2)
...
CALL U_MortarPrim(PP_nVarPrim,FV_multi_master,FV_multi_slave,doMPiSides=.FALSE.)
#endif
#endif
...
! 8.4)
CALL Flux_MortarCons(PP_nVar,Flux_master,Flux_slave,doMPISides=.FALSE.,weak=.TRUE.)
! 8.5)
CALL SurfIntCons(PP_nVar,PP_N,Flux_master,Flux_slave,Ut,.FALSE.,L_HatMinus,L_hatPlus)

#if USE_MPI
! 8.4)
CALL FinishExchangeMPIData(2*nNbProcs,MPIRequest_Flux )                                        ! Flux_slave: master -> slave
CALL Flux_MortarCons(PP_nVar,Flux_master,Flux_slave,doMPISides=.TRUE.,weak=.TRUE.)
! 8.5)
CALL SurfIntCons(PP_nVar,PP_N,Flux_master,Flux_slave,Ut,.TRUE.,L_HatMinus,L_HatPlus)
#endif /*USE_MPI*/
...
#if FV_ENABLED
  CALL ApplyJacobianCons(PP_nVar,Ut,toPhysical=.TRUE.,FVE=1)
#endif
ELSE
  CALL ApplyJacobianCons(PP_nVar,Ut,toPhysical=.TRUE.)
END IF
...
REAL                            :: UQuad(1:PP_nVarDet,1:nQPTotal)
...
DO iElem=1,nElems
  DO k=0,ZDIM(Nloc); DO j=0,Nloc; DO i=0,Nloc
    ...
    DO iQP=1,nQPTotal
      ...
      END DO
      CALL ExactFunc(IniExactFunc,0.,xGP(1:3,i,j,k,iElem),UQuad(:,iQP),xi)
    END DO
    ! project onto basis polynomials
    DO iVarDet=1,PP_nVarDet
      U(PP_iVarDet,i,j,k,iElem)=MATMUL(SG_Vdm_QuadOrth,UQuad(iVarDet,:))
    END DO
