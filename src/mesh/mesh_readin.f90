...
SWRITE(UNIT_StdOut,'(132("-"))')
! Open mesh file
CALL OpenDataFile(FileString,create=.FALSE.,single=.FALSE.,readOnly=.TRUE., communicator=MPI_COMM_SGELEM)
...
#if USE_MPI
IF(mySGElem.EQ.1) THEN 
  CALL MPI_REDUCE(ReduceData,ReduceData_glob,10,MPI_INTEGER,MPI_SUM,0,MPI_COMM_SGELEM,iError)
...
!==================================================================================================================================
SUBROUTINE BuildPartition(MeshFileIn)
! MODULES                                                                                                                          !
USE MOD_Globals
USE MOD_Mesh_Vars, ONLY:nElems,nGlobalElems,offsetElem,MeshFile
USE MOD_SG_Vars,   ONLY:mySGElem
#if USE_MPI
...
IMPLICIT NONE
! INPUT / OUTPUT VARIABLES
CHARACTER(LEN=255),INTENT(IN),OPTIONAL :: MeshFileIn
!-----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
#if USE_MPI
INTEGER           :: iElem
...
#endif
!===================================================================================================================================
IF(PRESENT(MeshFileIn)) MeshFile=TRIM(MeshFileIn)
...
IF(nGlobalElems.LT.nProcsPerSGElem(mySGElem)) THEN
  CALL Abort(__STAMP__,&
  'ERROR: Number of elements (1) is smaller then number of processors (2)!',nGlobalElems,REAL(nProcsPerSGElem(mySGElem)))
END IF

!simple partition: nGlobalelems/nprocs, do this on proc 0
IF(ALLOCATED(offsetElemMPI)) DEALLOCATE(offsetElemMPI)
ALLOCATE(offsetElemMPI(0:nProcsPerSGElem(mySGElem)))
offsetElemMPI=0
nElems=nGlobalElems/nProcsPerSGElem(mySGElem)
iElem=nGlobalElems-nElems*nProcsPerSGElem(mySGElem)
DO iProc=0,nProcsPerSGElem(mySGElem)-1
  offsetElemMPI(iProc)=nElems*iProc+MIN(iProc,iElem)
END DO
offsetElemMPI(nProcsPerSGElem(mySGElem))=nGlobalElems
!local nElems and offset
nElems=offsetElemMPI(mySGElemRank+1)-offsetElemMPI(mySGElemRank)
offsetElem=offsetElemMPI(mySGElemRank)
...
! MODULES
USE MOD_Globals,   ONLY:nProcsPerSGElem
USE MOD_SG_Vars,   ONLY:mySGElem
USE MOD_MPI_vars,  ONLY:offsetElemMPI
IMPLICIT NONE
!----------------------------------------------------------------------------------------------------------------------------------
...
!==================================================================================================================================
ELEMIPROC=0
maxSteps=INT(LOG(REAL(nProcsPerSGElem(mySGElem)))*1.4426950408889634556)+1    !1/LOG(2.)=1.4426950408889634556
low=0
up =nProcsPerSGElem(mySGElem)-1
...
END IF
DO i=1,mySGElem-1
  ELEMIPROC=ELEMIPROC+nProcsPerSGElem(i)
END DO 
...
CALL OpenDataFile(MeshFile,create=.FALSE.,single=.FALSE.,readOnly=.TRUE.,communicator=MPI_COMM_SGELEM)
