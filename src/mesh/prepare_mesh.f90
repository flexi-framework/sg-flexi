...
#if USE_MPI
IF(MPISGElemroot)THEN
  CALL MPI_REDUCE(MPI_IN_PLACE,nSides_flip,5,MPI_INTEGER,MPI_SUM,0,MPI_COMM_SGELEM,iError)
ELSE
  CALL MPI_REDUCE(nSides_flip,MPI_IN_PLACE,5,MPI_INTEGER,MPI_SUM,0,MPI_COMM_SGELEM,iError)
END IF
IF(MPISGElemroot)THEN
  CALL MPI_REDUCE(MPI_IN_PLACE     ,nSides_MortarType,3,MPI_INTEGER,MPI_SUM,0,MPI_COMM_SGELEM,iError)
ELSE
  CALL MPI_REDUCE(nSides_MortarType,MPI_IN_PLACE     ,3,MPI_INTEGER,MPI_SUM,0,MPI_COMM_SGELEM,iError)
END IF
#endif /*USE_MPI*/
