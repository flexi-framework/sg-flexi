...
DO iArgs = 2,nArgs
  ...
  IF(.NOT.ISVALIDHDF5FILE(InputStateFile,communicator=MPI_COMM_WORLD)) THEN
    CALL CollectiveStop(__STAMP__,&
  ...
  ! Open .h5 file
  CALL OpenDataFile(InputStateFile,create=.FALSE.,single=.FALSE.,readOnly=.TRUE.,communicator=MPI_COMM_WORLD)
  ! Read in parameters from the State file
  CALL GetDataProps(nVar_State,N_State,nElems_State,nSGElems_State,NodeType_State)
  ...
  IF (TRIM(MeshFile).NE.TRIM(MeshFile_old)) THEN
    ...
    IF(.NOT.ISVALIDMESHFILE(MeshFile,communicator=MPI_COMM_WORLD)) THEN
      CALL CollectiveStop(__STAMP__,&
