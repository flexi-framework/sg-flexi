...
INTERFACE WriteVarnamesToVTK_array_SG
  MODULE PROCEDURE WriteVarnamesToVTK_array_SG
END INTERFACE
...
PUBLIC::WriteVarnamesToVTK_array_SG
PUBLIC::CARRAY
!===================================================================================================================================
...
!> Subroutine to write variable names to VTK format
!===================================================================================================================================
SUBROUTINE WriteVarnamesToVTK_array_SG(nVar,varnames_out,VarNamesIn)
USE ISO_C_BINDING
! MODULES
USE MOD_Globals
USE MOD_StringTools    ,ONLY: STRICMP
...
TYPE (CARRAY), INTENT(INOUT)   :: varnames_out
CHARACTER(LEN=255),INTENT(IN)  :: VarNamesIn(nVar)
!-----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
CHARACTER(C_CHAR),POINTER    :: VarNames_loc(:,:)
INTEGER                      :: i,iVar
!===================================================================================================================================
! copy varnames
ALLOCATE(VarNames_loc(255,nVar))
varnames_out%len  = nVar*255
IF (nVar.GT.0) THEN
  varnames_out%data = C_LOC(VarNames_loc(1,1))
  
  DO iVar=1,nVar
    DO i=1,255
      VarNames_loc(i,iVar) = VarNamesIn(iVar)(i:i)
    END DO
  END DO
END IF

END SUBROUTINE WriteVarnamesToVTK_array_SG
