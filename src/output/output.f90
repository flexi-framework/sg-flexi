...
USE MOD_SG_Vars     , ONLY: t_HPLimiter,nLimitedElems
#endif
!----------------------------------------------------------------------------------------------------------------------------------!
! insert modules here
!----------------------------------------------------------------------------------------------------------------------------------!
...
#endif
#if SG_HP_LIMITER && USE_MPI
nLimitedElems(mySGElem)=COUNT(ANY(RESHAPE(t_HPLimiter,(/(PP_N+1)**PP_dim,nElems/)).GT.TINY(1.),1))
CALL MPI_ALLREDUCE(MPI_IN_PLACE,nLimitedElems(mySGElem),1,MPI_INTEGER,MPI_SUM,MPI_COMM_SGELEM,iError)
CALL MPI_GATHER(nLimitedElems(mySGElem),1,MPI_INTEGER,nLimitedElems,1,MPI_INTEGER,0,MPI_COMM_SGELEMROOT,iError)
#endif
...
#if FV_ENABLED
  FV_percent = REAL(FVcounter) / (nGlobalElems*nSGElems) * 100.
  WRITE(UNIT_stdOut,'(F6.2,A7)',ADVANCE='NO') FV_percent, '% FV | '
#endif
#if SG_HP_LIMITER
  DO iSGElem=1, nSGElems
    Limited_percent = REAL(nLimitedElems(iSGElem)) / nGlobalElems * 100.
    WRITE(UNIT_stdOut,'(F5.2,A1)',ADVANCE='NO') Limited_percent,'%'
  END DO
  WRITE(UNIT_stdOut,'(A)',ADVANCE='NO') ' ltd. | '
  WRITE(UNIT_stdOut,'(A,E10.4,A,E10.4,A,F6.2,A,I4,A1,I0.2,A1,I0.2,A1)',ADVANCE='NO') 'time = ', t, &
      ' | dt = ', dt, ' | ', percent, '% compl. | rem. time ~ ',INT(hours),':',INT(mins),':',INT(secs), ACHAR(13)
#else
  WRITE(UNIT_stdOut,'(A,E10.4,A,E10.4,A,F6.2,A,I4,A1,I0.2,A1,I0.2,A1)',ADVANCE='NO') 'Time = ', t, &
...
DO iElem=1,nElems
  ...
  ELSE
    ...
    DO k=0,PP_NZ; DO j=0,PP_N; DO i=0,PP_N
      CALL ConsToPrimSG(UPrim ,U(:,i,j,k,iElem))
      DO kk=0,PP_dim-2; DO jj=0,1; DO ii=0,1
        ...
        UPrim2 = UPrim
#endif
        CALL PrimToConsSG(UPrim2(1:PP_nVarPrim), FV_U_NVisu(:,iii,jjj,kkk,FV_iElem))
