...
IF (LEN_TRIM(RestartFile).GT.0) THEN
  ...
  validHDF5 = ISVALIDHDF5FILE(RestartFile,communicator=MPI_COMM_SGELEM)
  IF(.NOT.validHDF5) &
  ...
  CALL OpenDataFile(RestartFile,create=.FALSE.,single=.FALSE.,readOnly=.TRUE.,communicator=MPI_COMM_SGELEM)
  CALL GetDataProps(nVar_Restart,N_Restart,nElems_Restart,nSGElems_Restart,NodeType_Restart)
  ...
  ! dirty hack
#ifndef PP_M
  PP_nVar    =PP_nVarDet    *(PP_M+1)
  PP_nVarPrim=PP_nVarPrimDet*(PP_M+1)
#endif

  IF ((nVar_Restart.NE.PP_nVar).OR.(nElems_Restart.NE.nGlobalElems).OR.(nSGElems_Restart.NE.nSGElems)) THEN
...
IF(DoRestart)THEN
  ...
  CALL GetArrayAndName('ElemData','VarNamesAdd',nVal,tmp,VarNamesElemData,offset_stoch=mySGElem-1)
  ...
  WRITE(mySGElemString,'(I0)'),mySGElem
...
! Sanity check
  ...
  CALL ReadArray('DG_Solution',6,HSize_proc,OffsetElem,5,RealArray=U_local,offset_stoch=mySGElem-1)
  ...
  ELSE ! InterpolateSolution
    ...
    IF(N_Restart.GT.PP_N)THEN
      ...
      DEALLOCATE(JNR)
      ! Transform back
      CALL ApplyJacobianCons(PP_nVar,U,toPhysical=.TRUE.)
    ELSE
      DO iElem=1,nElems
        IF (FV_Elems(iElem).EQ.0) THEN ! DG element
          CALL ChangeBasisVolume(PP_nVar,N_Restart,PP_N,Vdm_NRestart_N,U_local(:,:,:,:,iElem),U(:,:,:,:,iElem))
