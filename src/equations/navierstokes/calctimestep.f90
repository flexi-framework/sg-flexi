...
!----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
INTEGER                          :: i,j,k,iElem,iVarDet,iQP
...
DO iElem=1,nElems
  ...
  DO k=0,PP_NZ; DO j=0,PP_N; DO i=0,PP_N
    ...
    DO iVarDet=1,PP_nVarDet
      UQuad(iVarDet,:)=MATMUL(SG_VdM_OrthQuad,U(PP_iVarDet,i,j,k,iElem))
    END DO
    ...
    DO iQP=1,nQPTotal
      ...
      UE(DPRES)=PRESSURE_HE(UE)
      UE(DTEMP)=TEMPERATURE_HE(UE)
      ! Convective Eigenvalues
      IF(IEEE_IS_NAN(UE(DDENS)))THEN
        ERRWRITE(*,'(A,3ES16.7)')'Density NaN, Position= ',Elem_xGP(:,i,j,k,iElem)
        errType=1
      END IF
      c=SPEEDOFSOUND_HE(UE)
      vsJ=UE(DVELV)*sJ(i,j,k,iElem,FVE)
...
#else
dtElem(iElem)=CFLScale(FVE)*2./(SUM(Max_Lambda)*SG_SafetyFactor)
#endif
