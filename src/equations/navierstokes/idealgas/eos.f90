...
USE MOD_SG_Vars       ,ONLY: viscosityDim,FullOrderToTensorVec,Distribution,StochMu,StochSigma,UniIntBounds
#if PP_VISC == 1
...
INTEGER             :: MultiInd(PP_nDimStoch),iCoef,iCoefLinear
!==================================================================================================================================
...
#if PARABOLIC
ALLOCATE(mu0(PP_nCoef))
mu0=0.

! mu0 only has two non zero entries: 
! - Expectation: Psi_0 in all dimensions
! - StdDeviation: Psi_1 in mu-direction, Psi_0 in all others. 
! The index of the first one is trivial, the second one (iCoefLinear) has to be searched for

! Search for 1D-index of linear entry of mu0
MultiInd = 0
MultiInd(viscosityDim)=1
iCoefLinear=0
DO iCoef=1,PP_nCoefM1
  IF(ALL(FullOrderToTensorVec(:,iCoef).EQ.MultiInd))THEN
    !+1, sind mu0 runs from 1 to PP_nCoef and FullOrderToTensorVec runs from 0 to PP_nCoefM1
    iCoefLinear = iCoef + 1 
    EXIT
  END IF 
END DO 
IF((PP_M.GT.0).AND.(iCoefLinear.EQ.0)) CALL Abort(__STAMP__,'Linear Coefficient of mu0 not found')

! Set non-zero entries of mu0
SELECT CASE(Distribution)
CASE(SG_DIST_NORMAL)
  mu0(1) = StochMu(viscosityDim)
  IF(PP_M.GT.0) mu0(iCoefLinear) = StochSigma(viscosityDim)
CASE(SG_DIST_UNIFORM)
  mu0(1) = 0.5*SUM(UniIntBounds(:,viscosityDim))
  IF(PP_M.GT.0) mu0(iCoefLinear) = (UniIntBounds(2,viscosityDim)-UniIntBounds(1,viscosityDim))/SQRT(12.)
END SELECT

Pr       =GETREAL('Pr','0.72')
KappasPr =Kappa/Pr

!FOLLOWING PART IS NOT NEEDED. mu0 is always stochastic

!! Viscosity
!#if   PP_VISC == 0
!! Constant viscosity
!IF(.NOT. UseNonDimensionalEqn)THEN
  !mu0(1)=GETREAL('mu0','0.0')
!ELSE
  !mu0(1)=1./BulkReynolds
  !SWRITE(UNIT_stdOut,'(A,ES16.7)')' |                            mu0 | Set to 1/BulkReynolds=',mu0
!END IF
!#elif PP_VISC == 1
!! mu-Sutherland
!mu0(1)     =GETREAL('mu0','1.735E-5')
!Ts      =GETREAL('Ts','110.4')
