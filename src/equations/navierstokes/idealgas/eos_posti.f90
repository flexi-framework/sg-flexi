...
USE MOD_Equation_Vars ,ONLY: StrVarNamesDet
...
GetMaskCons = 0
DO iVar=1,nVarDepEOS
  DO iVar2=1,PP_nVarDet
    IF (STRICMP(StrVarNamesDet(iVar2),DepNames(iVar))) THEN
      GetMaskCons(iVar) = 1
    END IF
  END DO
END DO
...
! MODULES
USE MOD_EOS_Posti_Vars,ONLY: nVarDepEOS,DepTableEOS,DepNames
USE MOD_Equation_Vars ,ONLY: StrVarNamesPrimDet
USE MOD_StringTools   ,ONLY: STRICMP
IMPLICIT NONE
!----------------------------------------------------------------------------------------------------------------------------------!
! INPUT / OUTPUT VARIABLES
...
GetMaskPrim = 0
DO iVar=1,nVarDepEOS
  DO iVar2=1,PP_nVarPrimDet
    IF (STRICMP(StrVarNamesPrimDet(iVar2),DepNames(iVar))) THEN
      GetMaskPrim(iVar) = 1
    END IF
  END DO
END DO
...
REAL,DIMENSION(1:PP_nVarPrimDet,PRODUCT(nVal)),INTENT(IN),OPTIONAL :: gradUx,gradUy,gradUz
REAL,DIMENSION(1:3,PRODUCT(nVal)),INTENT(IN),OPTIONAL           :: NormVec,TangVec1,TangVec2
!-----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
...
SELECT CASE(DepName_low)
  ...
  CASE("pressure")
    DO i=1,PRODUCT(nVal)
      UE(DSRHO) = 1./UCalc(i,iDens)
      UE(DMOM1) =    UCalc(i,iMom1)
      UE(DMOM2) =    UCalc(i,iMom2)
      UE(DMOM3) =    UCalc(i,iMom3)
      UE(DENER) =    UCalc(i,iEner)
      UE(DVELV) = UE(DSRHO)*UE(DMOMV)
      UCalc(i,iVarCalc) = PRESSURE_HE(UE)
    END DO
  CASE("temperature")
    DO i=1,PRODUCT(nVal)
      UE(DSRHO) = 1./UCalc(i,iDens)
      UE(DPRES) =    UCalc(i,iPres)
  ...
  CASE("pressuretimederiv")
    CALL Abort(__STAMP__,'Pressure Time Derivative not implemented')
...
DO i=1,PRODUCT(nVal)
  ...
  DO l=1,3
    DO k=1,3
      ! shear rate
      S=0.5*(gradUmat(k,l)+gradUmat(l,k))
      ! rotation rate
      Rot=0.5*(gradUmat(k,l)-gradUmat(l,k))
