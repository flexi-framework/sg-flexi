...
USE MOD_Equation_Vars     ,ONLY: nRefState,BCData,BCDataPrim,nBCByType,BCSideID,BCData_Quad,BCDataPrim_Quad
...
#endif
USE MOD_EOS               ,ONLY: ConsToPrim,ConsToPrimDet
USE MOD_ExactFunc         ,ONLY: ExactFunc
USE MOD_SG_Vars           ,ONLY: nQPTotal,xiQP_nDim,xiDet,nDimStochPositions
IMPLICIT NONE
!----------------------------------------------------------------------------------------------------------------------------------
! INPUT / OUTPUT VARIABLES
!----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
INTEGER :: i,iSide,iQP,iDimStoch
INTEGER :: locType,locState
INTEGER :: MaxBCState,MaxBCStateGlobal
LOGICAL :: readBCdone
REAL    :: talpha,tbeta
INTEGER :: p,q
REAL    :: xiAtQuad(1:SGV_ARRAY_LEN)
!==================================================================================================================================
...
ALLOCATE(BCData(PP_nVarDet,        0:PP_N,0:PP_NZ,nBCSides))
ALLOCATE(BCDataPrim(PP_nVarPrimDet,0:PP_N,0:PP_NZ,nBCSides))
BCData=0.
BCDataPrim=0.
ALLOCATE(BCData_Quad(PP_nVarDet,        0:PP_N,0:PP_NZ,nBCSides,1:nQPTotal))
ALLOCATE(BCDataPrim_Quad(PP_nVarPrimDet,0:PP_N,0:PP_NZ,nBCSides,1:nQPTotal))
BCData_Quad=0.
BCDataPrim_Quad=0.
  ...
  IF (Boundarytype(BC(iSide),BC_TYPE).EQ.121) THEN
    DO q=0,PP_NZ; DO p=0,PP_N
      DO iQP=1,nQPTotal
        ...
        END DO
        CALL ExactFunc(Boundarytype(BC(iSide),BC_STATE),0.,Face_xGP(:,p,q,0,iSide),BCData_Quad(:,p,q,iSide,iQP),xiAtQuad)
        CALL ConsToPrimDet(BCDataPrim_Quad(:,p,q,iSide,iQP),BCData_Quad(:,p,q,iSide,iQP))
      END DO
...
!==================================================================================================================================
SUBROUTINE GetBoundaryState(SideID,t,Nloc,UPrim_boundary,UPrim_master,NormVec,TangVec1,TangVec2,Face_xGP,iQP)
!----------------------------------------------------------------------------------------------------------------------------------!
! MODULES                                                                                                                          !
USE MOD_PreProc
...
USE MOD_ExactFunc    ,ONLY: ExactFunc
USE MOD_Equation_Vars,ONLY: IniExactFunc,BCDataPrim,RefStatePrim,nRefState,BCDataPrim_Quad
USE MOD_SG_Vars      ,ONLY: xiQP_nDim,xiDet,nDimStochPositions
!----------------------------------------------------------------------------------------------------------------------------------!
! insert modules here
!----------------------------------------------------------------------------------------------------------------------------------!
IMPLICIT NONE
...
REAL,INTENT(IN)             :: UPrim_master(  PP_nVarPrimDet,0:Nloc,0:ZDIM(Nloc)) !< inner surface solution
...
REAL,INTENT(OUT)            :: UPrim_boundary(PP_nVarPrimDet,0:Nloc,0:ZDIM(Nloc)) !< resulting boundary state
Integer,INTENT(IN),OPTIONAL :: iQP

! INPUT / OUTPUT VARIABLES
!-----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
INTEGER                     :: p,q,iDimStoch
REAL                        :: absdiff(1:nRefState)
INTEGER                     :: BCType,BCState
REAL,DIMENSION(PP_nVarDet)  :: Cons
REAL                        :: MaOut
...
CASE(121) ! exact BC = Dirichlet BC !!
  ! SPECIAL BC: BCState uses Exact func computed once at the beginning (121)
  ! Dirichlet means that we use the gradients from inside the grid cell
  UPrim_boundary(:,:,:) = BCDataPrim_Quad(:,:,:,SideID,iQP)
CASE(22) ! exact BC = Dirichlet BC !!
  ...
  DO q=0,ZDIM(Nloc); DO p=0,Nloc
    ...
    DO iDimStoch=1,PP_nDimStoch
      xiAtQuad(nDimStochPositions(iDimStoch)) = xiQP_nDim(iDimStoch,iQP)
    END DO
    CALL ExactFunc(BCState,t,Face_xGP(:,p,q),Cons,xi=xiAtQuad)
    CALL ConsToPrimDet(UPrim_boundary(:,p,q),Cons)
  END DO; END DO
...
CASE(3,4,9,23,24,25,27)
  ...
  DO q=0,ZDIM(Nloc); DO p=0,Nloc
    ...
    UPrim_boundary(5:PP_nVarPrimDet,p,q)= UPrim_master(5:PP_nVarPrimDet,p,q)
  END DO; END DO !p,q
...
USE MOD_Globals      ,ONLY: Abort
USE MOD_Mesh_Vars    ,ONLY: BoundaryType,BC
USE MOD_EOS          ,ONLY: PrimToConsSG,ConsToPrimSG
USE MOD_ExactFunc    ,ONLY: ExactFunc
#if PARABOLIC
...
USE MOD_SG_Vars      ,ONLY: nQPTotal,SG_Vdm_QuadOrth,SG_Vdm_OrthQuad
!----------------------------------------------------------------------------------------------------------------------------------
! INPUT / OUTPUT VARIABLES
INTEGER,INTENT(IN)   :: SideID
REAL,INTENT(IN)      :: t       !< current time (provided by time integration scheme)
...
#endif /*PARABOLIC*/
REAL                                 :: UPrim_boundary_Quad(PP_nVarPrimDet,0:Nloc,0:ZDIM(Nloc),nQPTotal)
INTEGER                              :: iVarDet,iQP
REAL                                 :: UPrim_master_Quad(PP_nVarPrimDet,0:Nloc,0:ZDIM(Nloc),nQPTotal)
!==================================================================================================================================
BCType  = Boundarytype(BC(SideID),BC_TYPE)
BCState = Boundarytype(BC(SideID),BC_STATE)
...
IF (BCType.LT.0) THEN ! testcase boundary condition
  CALL GetBoundaryFluxTestcase(SideID,t,Nloc,Flux,UPrim_master,              &
                               ...
                               NormVec,TangVec1,TangVec2,Face_xGP)
ELSE
  ! Evaluate at stoch quadrature points
  DO q=0,ZDIM(Nloc); DO p=0,Nloc
    DO iVarDet=1,PP_nVarPrimDet
      UPrim_master_Quad(iVarDet,p,q,:)=MATMUL(SG_VdM_OrthQuad,UPrim_master(PP_iVarDet,p,q))
    END DO
  END DO; END DO
  DO iQP=1,nQPTotal
    CALL GetBoundaryState(SideID,t,Nloc,UPrim_boundary_Quad(:,:,:,iQP),UPrim_master_Quad(:,:,:,iQP),&
        NormVec,TangVec1,TangVec2,Face_xGP,iQP)
  END DO
  ! project onto basis polynomials
  DO q=0,ZDIM(Nloc); DO p=0,Nloc
    DO iVarDet=1,PP_nVarPrimDet
      UPrim_boundary(PP_iVarDet,p,q)=MATMUL(SG_Vdm_QuadOrth,UPrim_boundary_Quad(iVarDet,p,q,:))
    END DO
  END DO; END DO ! p,q=0,PP_N
  ...
  CASE(2,12,121,22,23,24,25,27) ! Riemann-Type BCs
    DO q=0,ZDIM(Nloc); DO p=0,Nloc
      CALL PrimToConsSG(UPrim_master(:,p,q), UCons_master(:,p,q))
      CALL PrimToConsSG(UPrim_boundary(:,p,q),      UCons_boundary(:,p,q))
    END DO; END DO ! p,q=0,PP_N
  ...
  CASE(3,4,9) ! Walls
    ...
    DO q=0,ZDIM(Nloc); DO p=0,Nloc
      ...
      Flux(SGI1,p,q) = 0.
      Flux(SGI2,p,q) = UPrim_boundary(SGI5,p,q)*NormVec(1,p,q)
      Flux(SGI3,p,q) = UPrim_boundary(SGI5,p,q)*NormVec(2,p,q)
      Flux(SGI4,p,q) = UPrim_boundary(SGI5,p,q)*NormVec(3,p,q)
      Flux(SGI5,p,q) = 0.
    END DO; END DO !p,q
    ! Diffusion
#if PARABOLIC
    ...
    ! All variables except for Dens (iVarDet=2,nVarDet)
    DO q=0,ZDIM(Nloc); DO p=0,Nloc
      DO iVarDet=2,PP_nVarDet
        Flux(PP_iVarDet,p,q) =  Flux(PP_iVarDet,p,q) + &
          NormVec(1,p,q)*Fd_Face_loc(PP_iVarDet,p,q) + &
          NormVec(2,p,q)*Gd_Face_loc(PP_iVarDet,p,q) + &
          NormVec(3,p,q)*Hd_Face_loc(PP_iVarDet,p,q)
      END DO ! ivarDet
    END DO; END DO !p,q
#endif /*PARABOLIC*/
...
USE MOD_SG_Vars       ,ONLY: nQP, SG_Vdm_OrthQuad, SG_Vdm_QuadOrth
IMPLICIT NONE
!----------------------------------------------------------------------------------------------------------------------------------
! INPUT / OUTPUT VARIABLES
INTEGER,INTENT(IN):: SideID
...
REAL              :: UPrim_boundary_Quad(PP_nVarPrimDet,0:PP_N,0:PP_NZ,nQP)
REAL              :: UPrim_master_Quad(  PP_nVarPrimDet,0:PP_N,0:PP_NZ,nQP)
...
ELSE
  ...
  END DO; END DO
  DO iQP=1,nQP
    CALL GetBoundaryState(SideID,t,PP_N,UPrim_boundary_Quad(:,:,:,iQP),UPrim_master_Quad(:,:,:,iQP),&
                          NormVec,TangVec1,TangVec2,Face_xGP,iQP)
  END DO
  ! project onto basis polynomials
  DO q=0,PP_NZ; DO p=0,PP_N
...
REAL              :: UPrim_boundary_Quad(PP_nVarPrimDet,0:PP_N,0:PP_NZ,nQPTotal)
REAL              :: UPrim_master_Quad(PP_nVarPrimDet,0:PP_N,0:PP_NZ,nQPTotal)
INTEGER           :: iVarDet,iQP
!==================================================================================================================================
BCType  = Boundarytype(BC(SideID),BC_TYPE)
BCState = Boundarytype(BC(SideID),BC_STATE)
...
ELSE
  ...
  CASE(3,4) ! No-slip wall BCs
    DO q=0,PP_NZ; DO p=0,PP_N
      Flux(SGI1,p,q)  = UPrim_Boundary(SGI1,p,q)
      Flux(PVELV,p,q) = 0.
      Flux(SGI5,p,q)  = UPrim_Boundary(SGI5,p,q)
      Flux(SGI6,p,q)  = UPrim_Boundary(SGI6,p,q)
  ...
  CASE(9)
    ...
    DO q=0,PP_NZ; DO p=0,PP_N
      ! Compute Flux
      Flux(SGI1,p,q)  = UPrim_master(SGI1,p,q)
      Flux(PVELV,p,q) = UPrim_boundary(PVELV,p,q)
      Flux(SGI5,p,q)  = UPrim_master(SGI5,p,q)
      Flux(SGI6,p,q)  = UPrim_master(SGI6,p,q)
    END DO; END DO !p,q
  CASE(1) !Periodic already filled!
  CASE DEFAULT ! unknown BCType
    CALL abort(__STAMP__,&
...
INTEGER                       :: nVar_HDF5,N_HDF5,nElems_HDF5,N_HDF5Z,nSGElems_HDF5
...
CALL OpenDataFile(FileName,create=.FALSE.,single=.FALSE.,readOnly=.TRUE.,communicator=MPI_COMM_SGELEM)
CALL GetDataProps(nVar_HDF5,N_HDF5,nElems_HDF5,nSGElems_HDF5,NodeType_HDF5)
...
! MODULES
USE MOD_Equation_Vars,ONLY: BCData,BCDataPrim,nBCByType,BCSideID,BCDataPrim_Quad,BCData_Quad
IMPLICIT NONE
!----------------------------------------------------------------------------------------------------------------------------------
! INPUT / OUTPUT VARIABLES
!----------------------------------------------------------------------------------------------------------------------------------
...
!==================================================================================================================================
SDEALLOCATE(BCData)
SDEALLOCATE(BCData_Quad)
SDEALLOCATE(BCDataPrim_Quad)
