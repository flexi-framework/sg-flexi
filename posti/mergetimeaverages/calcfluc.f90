...
USE MOD_IO_HDF5
USE MOD_Mesh_Vars,  ONLY:nGlobalElems,nElems,nMeshes
...
INTEGER                        :: nValMean(15),nValMeanSquare(15),rank
...
DO iArg=1,nArgs
  ...
  !Output
  CALL OpenDataFile(InputFile,create=.FALSE.,single=.FALSE.,readOnly=.FALSE.,communicator=MPI_COMM_WORLD)
  CALL WriteArray('Fluc',rank,nValMean,nValMean,0*nValMean,collective=.TRUE.,RealArray=UFluc)
...
CALL OpenDataFile(InputFile,create=.FALSE.,single=.FALSE.,readOnly=.TRUE.,communicator=MPI_COMM_WORLD)
CALL GetDataProps(nVar_State,NState,nGlobalElems,nMeshes)
nElems=nGlobalElems
...
rank=COUNT(nValMean.GT.0)
nVarMean=nValMean(1)
nVarMeanSquare=nValMeanSquare(1)
nDOF=PRODUCT(nValMean(2:rank))
