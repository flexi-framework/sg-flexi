      ...
      vtkInformationVector *outputVector)
{
   ...
   dprintf(posti_unit, "CalcMean = %s\n", (this->CalcMean ? "T" : "F"));
   dprintf(posti_unit, "CalcStd = %s\n", (this->CalcStd ? "T" : "F"));
   dprintf(posti_unit, "CalcUofXi = %s\n", (this->CalcUofXi ? "T" : "F"));
   dprintf(posti_unit, "nQP = %d\n", nQP); // insert NVisu
   dprintf(posti_unit, "CustomnQP = %s\n", (this->CustomnQP ? "T" : "F"));

   // Divide xiEval input string into substrings and write them individually
   std::string       segment;
   std::stringstream input_string(this->xiEval);
   while(std::getline(input_string, segment, ','))  // ',' as data separator
   {
      dprintf(posti_unit, "xiEval =  %s\n",segment.c_str()); 
   }
