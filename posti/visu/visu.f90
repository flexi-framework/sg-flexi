...
IF (ISVALIDMESHFILE(statefile,communicator=MPI_COMM_WORLD)) THEN      ! MESH
  SDEALLOCATE(varnames_loc)
  FileType='Mesh'
ELSE IF (ISVALIDHDF5FILE(statefile,communicator=MPI_COMM_WORLD)) THEN ! other file
  SDEALLOCATE(varnames_loc)
  ...
  IF (STRICMP(FileType,'State')) THEN
    ...
    CALL GetVarNames("VarNamesDet",VarNamesHDF5,VarNamesExist)
    IF (VarNamesExist) THEN
      IF (PP_nVarDet.EQ.SIZE(VarNamesHDF5)) THEN
        sameVars=.TRUE.
    ...
    ELSE
      IF (STRICMP(datasetNames(i), "DG_Solution")) THEN
        IF (readDGsolutionVars) THEN
          CALL GetVarNames("VarNamesDet",varnames_tmp,VarNamesExist)
        END IF
  ...
  IF (file_exists) THEN
    ...
    CALL OpenDataFile(MeshFile_loc,create=.FALSE.,single=.FALSE.,readOnly=.TRUE.,communicator=MPI_COMM_WORLD)
...
USE MOD_ReadInTools        ,ONLY: prms,GETINT,GETLOGICAL,GETSTR,GETREAL
USE MOD_ReadInTools        ,ONLY: addStrListEntry,CountOption,FinalizeParameters
...
USE MOD_SG_Vars            ,ONLY: StochVarNames,nSGElems,nQPVisu
IMPLICIT NONE
CHARACTER(LEN=255),INTENT(IN)    :: statefile
CHARACTER(LEN=255),INTENT(INOUT) :: postifile
!-----------------------------------------------------------------------------------------------------------------------------------
! LOCAL
INTEGER                          :: nElems_State,iDimStoch,nSGElems_State,nDimStoch_State
CHARACTER(LEN=255)               :: NodeType_State, cwd,str
!===================================================================================================================================
...
!IF (changedStateFile.OR.changedMeshFile) THEN
  CALL GetDataProps(nVar_State,PP_N,nElems_State,nSGElems_State,NodeType_State)
  ...
  ! Read some SG information in any case for information printed to log
  CALL ReadAttribute(File_ID,'PP_nDimStoch' ,1,IntScalar = nDimStoch_State)
  SDEALLOCATE(StochVarNames)
  ALLOCATE(StochVarNames(nDimStoch_State))
  CALL ReadAttribute(File_ID,'StochVarNames',1,StrArray  = StochVarNames)
  CALL ReadAttribute(File_ID,'nSGElems'     ,1,IntScalar = nSGElems)
  ! If the polynomial degree is changing, we could need new mesh mappings.
  IF (NState_old.NE.PP_N) changedMeshFile = .TRUE.
!END IF
CALL CloseDataFile()

IF (GETLOGICAL("CustomnQP" ,'F')) THEN
  nQPVisu = GETINT('nQP','-1')
ELSE 
  nQPVisu = -1
END IF

doCalcMean  = GETLOGICAL("CalcMean" ,'T')
doCalcStd   = GETLOGICAL("CalcStd"  ,'F')
doCalcUofXi = GETLOGICAL("CalcUofXi",'F')
doCalcStoch=(/doCalcMean,doCalcStd,doCalcUofXi/)
! Write Xi evaluation point to log
IF (doCalcUofXi) THEN
  SDEALLOCATE(xiEval)
  ALLOCATE(xiEval(nDimStoch_State))
  xiEval(:) = 0.
  ! Read-in as multiple instead of array to avoid crash when wrong dimension of input in paraview-plugin
  DO iDimStoch=1,MIN(nDimStoch_State,CountOption("xiEval"))
    xiEval(iDimStoch) = GETREAL("xiEval",'0.')
  END DO

  SWRITE(UNIT_StdOut,'(132("#"))')
  SWRITE(UNIT_StdOut,'(132("#"))')
  SWRITE(UNIT_StdOut,*) "[SG]  Stochastic variables:"

  DO iDimStoch=1,nDimStoch_State
    WRITE(str,'(F7.4)') xiEval(iDimStoch)
    IF (iDimStoch.GT.CountOption("xiEval")) THEN
      CALL PrintWarning('For the following stochastic variables no xi to evaluate at is set! (Therefore Xi = 0. is chosen.)')
    END IF
    SWRITE(*,'(A30,A)')   TRIM(StochVarNames(iDimStoch)),' evaluated at Xi = '//TRIM(str)
  END DO

  ! More xi entries given than stochastic variables in state file
  IF (CountOption("xiEval").GT.nDimStoch_State) THEN
    SWRITE(str,*) nDimStoch_State
    CALL PrintWarning(TRIM("More entries in xiEval set then stochastic Variables in State-File. "//&
                           "Number of stochastic variables: "//TRIM(str)))
  END IF
  SWRITE(UNIT_StdOut,'(132("#"))')
  SWRITE(UNIT_StdOut,'(132("#"))')
END IF
...
!USE MOD_Posti_Calc          ,ONLY: CalcSurfQuantities_DG
USE MOD_Posti_Calc          ,ONLY: TransformWrapper
#if FV_ENABLED
!USE MOD_Posti_Calc          ,ONLY: CalcSurfQuantities_FV
#endif
USE MOD_Posti_ConvertToVisu ,ONLY: ConvertToVisu_GenericData
!USE MOD_Posti_ConvertToVisu ,ONLY: ConvertToSurfVisu_DG
...
CALL prms%CreateLogicalOption(  "CalcMean"        , "Calculate mean of SG solution",'T')
CALL prms%CreateLogicalOption(  "CalcStd"         , "Calculate standard deviation of SG solution",'F')
CALL prms%CreateLogicalOption(  "CalcUofXi"       , "Calculate SG solution evaluated at certain xi",'F')
CALL prms%CreateRealOption(     "xiEval"          , "xi for solution to be evaluated at (normalized)",multiple=.TRUE.)
CALL prms%CreateLogicalOption(  "CustomnQP"    , "Custom Number of stochastic integration points",'F')
CALL prms%CreateIntOption(      "nQP"          , "Number of stochastic integration points if desired to be changed",'-1')
...
IF (ISVALIDMESHFILE(statefile,communicator=MPI_COMM_WORLD)) THEN ! visualize mesh
...
ELSE IF (ISVALIDHDF5FILE(statefile,communicator=MPI_COMM_WORLD)) THEN ! visualize state file
  SWRITE(*,*) "State Mode"
  MeshFileMode = .FALSE.
  ...
  CALL GetVarNamesVisuFromVarNamesAll()
...
!>
!===================================================================================================================================
SUBROUTINE GetVarNamesVisuFromVarNamesAll()
! MODULES                                                                                                                          !
USE MOD_Visu_Vars
...
!-----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
INTEGER             :: iVarMult,iVar,iSGElem,iVarAdd
CHARACTER(LEN=255)  :: mySGElemString
!===================================================================================================================================
SDEALLOCATE(VarNamesVisu)
ALLOCATE(VarNamesVisu(nVarVisu))
DO iVar=1,nVarDepEOS
  IF (mapAllVarsToVisuVars(iVar).GT.0) THEN
    iVarMult=mapAllVarsToVisuVars(iVar)
    IF(doCalcMean) THEN
      VarNamesVisu(iVarMult) = TRIM(VarnamesAll(iVar))//"_Mean"
      iVarMult=iVarMult+nVarMult
    END IF
    IF(doCalcStd) THEN
      VarNamesVisu(iVarMult) = TRIM(VarnamesAll(iVar))//"_Std"
      iVarMult=iVarMult+nVarMult
    END IF
    IF(doCalcUofXi) THEN
      VarNamesVisu(iVarMult) = TRIM(VarnamesAll(iVar))//"_UofXi"
    END IF
  END IF
END DO
DO iVar=nVarDepEOS+1,nVarAll
  IF (mapAllVarsToVisuVars(iVar).GT.0) THEN
    iVarAdd= mapAllVarsToVisuVars(iVar) - nVarMult
    iVarAdd= nMult*nVarMult+(iVarAdd-1)*nSGElems
    DO iSGElem=1,nSGElems
      WRITE(mySGElemString,'(I0)') iSGElem
      VarNamesVisu(iVarAdd+iSGElem) = TRIM(VarnamesAll(iVar))//"_"//TRIM(mySGElemString)
    END DO 
  END IF
END DO
END SUBROUTINE GetVarNamesVisuFromVarNamesAll
...
SDEALLOCATE(UCalcMean)
SDEALLOCATE(UCalcStd)
