...
SDEALLOCATE(U)
ALLOCATE(U(1:nVar_State,0:PP_N,0:PP_N,0:PP_NZ,nElems,nSGElems))
CALL OpenDataFile(statefile,create=.FALSE.,single=.FALSE.,readOnly=.TRUE.,communicator=MPI_COMM_WORLD)
CALL ReadArray('DG_Solution',6,(/nVar_State,PP_N+1,PP_N+1,PP_NZ+1,nElems,nSGElems/),offsetElem,5,RealArray=U)
CALL CloseDataFile()
